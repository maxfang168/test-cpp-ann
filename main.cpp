...
int runANN()
{
	...
	double scaleFactor = 10.00; // Strengthens input siginals
	int outputIndex;			 // Index of output in array of board
	// Input neurons:
	for (int i = 0; i < 9; i++)
	{
		inputValues[i] = board[i] * scaleFactor * inputWeights[i];
	}
	// layer1Values[i] = inputValues[i] * layer1Weights[i] + layer1Biases[i];
	// Layer 1 w/act funct
	for (int i = 0; i < 9; i++)
	{
		layer1Values[i] = ((inputValues[0] + inputValues[1] + inputValues[2] + inputValues[3] + inputValues[4] + inputValues[5] + inputValues[6] + inputValues[7] + inputValues[8]) / 9) * layer1Weights[i] + layer1Biases[i];
		if (layer1Values[i] <= minValue)
		{ // Activation function
			layer1Values[i] = 0;
		}
	}
...
{
	long double adup = 4 / (1 + (static_cast<long double>(gameNumber) / 200)); // Number to add up for win.
	long double addo = 4 / (1 + (static_cast<long double>(gameNumber) / 101)); // Number to subtract for loss.
	long double addr = 2 / (2 + (static_cast<long double>(gameNumber) / 400)); // Number to subtract for draw.
...
// mainly used in bot() to check if board is empty.
bool boardIsEmpty()
{
...
// checks if certain board index is empty
bool boardIndexIsEmpty(int index)
{
	if (board[index] == 0)
	{
		return true;
...
// checks win from manually inputted board (not global var) Returns char (a, h, or d) a == ann win, h == human win, d == draw. Pieces in parameter board: a = -1, h = 1
char checkWinFromManualBoard(int newBoard[9])
{
	// check columns
	if (newBoard[0] == newBoard[1] && newBoard[1] == newBoard[2] && newBoard[1] != 0)
	{
		if (newBoard[1] == -1)
		{
			return 'a';
	...
	else if (newBoard[3] == newBoard[4] && newBoard[4] == newBoard[5] && newBoard[4] != 0)
	{
		if (newBoard[4] == -1)
		{
			return 'a';
	...
	else if (newBoard[6] == newBoard[7] && newBoard[7] == newBoard[8] && newBoard[7] != 0)
	{
		if (newBoard[7] == -1)
		{
			return 'a';
	...
	// check diagonals
	else if (newBoard[0] == newBoard[4] && newBoard[4] == newBoard[8] && newBoard[4] != 0)
	{
		if (newBoard[4] == -1)
		{
			return 'a';
	...
	else if (newBoard[2] == newBoard[4] && newBoard[4] == newBoard[6] && newBoard[4] != 0)
	{
		if (newBoard[4] == -1)
		{
			return 'a';
	...
	// check draw (newBoard is full)
	else if (newBoard[0] != 0 && newBoard[1] != 0 && newBoard[2] != 0 && newBoard[3] != 0 && newBoard[4] != 0 && newBoard[5] != 0 && newBoard[6] != 0 && newBoard[7] != 0 && newBoard[8] != 0)
	{
		return 'd';
	}
	else
...
// Manually coded bot. Doesn't use ANN. Used to train. NOtE: This only checks for one move at a time (depth one), meaning it looks one move into the future.
int bot()
{
	int move = -1;
	if (boardIsEmpty() == true) // Check if board is empty for first move
	{
		move = 4;
	}
	else
	{
		if (boardIndexIsEmpty(4) == true)
		{
			move = 4;
		}
		float scores[9] = {0, 0, 0, 0, 0, 0, 0, 0, 0}; // manages move selection by rating each move. (0 means no win, 0.5 means draw, -1 means loss, 1 means win)
		// check if bot can win
		for (int i = 0; i < 9; i++)
		{
			if (boardIndexIsEmpty(i) == true)
			{
				int tempBoard[9];
				for (int j = 0; j < 9; j++)
				{
				...
				tempBoard[i] = 1;
				if (checkWinFromManualBoard(tempBoard) == 'h')
				{
					scores[i] = 1;
				}
				else if (checkWinFromManualBoard(tempBoard) == 'a')
				{
					scores[i] = -1;
				}
				else if (checkWinFromManualBoard(tempBoard) == 'd')
				{
					scores[i] = 0.5;
				}
				else if (checkWinFromManualBoard(tempBoard) == 'n')
				{
					scores[i] = 0;
				}
			}
		}
		// find first move that can win
		for (int i = 0; i < 9; i++)
		{
			if (scores[i] == 1)
			{
		...
		//find first move that is in empty square (last resort)
		for (int i = 0; i < 9; i++)
		{
			if (boardIndexIsEmpty(i) == true)
			{
